...
export async function loadCloudData(userId: string) {
  ...
  if (!motorcycle) {
    ...
    const seeded = await supabase.rpc("seed_adv150_items", {
      p_motorcycle_id: motorcycle.id,
    });
  ...
  const [itemsResult, recordsResult] = await Promise.all([
    ...
    supabase
      ...
      .select("*, maintenance_record_items(maintenance_item_id)")
      .eq("motorcycle_id", motorcycle.id)
  ...
  return {
    motorcycle: {
      ...
      startDate: motorcycle.start_date ?? "",
      currentOdometerKm: motorcycle.current_odometer_km,
      id: motorcycle.id,
    },
    ...
    records: (recordsResult.data ?? []).map((record) => ({
      id: record.id,
      itemIds: record.maintenance_record_items?.map(
        (row: { maintenance_item_id: string }) => row.maintenance_item_id,
      ) ?? [record.maintenance_item_id],
      performedDate: record.performed_date,
      odometerKm: record.odometer_km,
      costSen: record.cost_sen ?? undefined,
...
export async function saveRecord(
  ...
  if (!record.itemIds.length)
    throw new Error("Choose at least one maintenance item.");
  const { error } = await supabase.rpc("save_maintenance_record", {
    p_id: record.id,
    p_motorcycle_id: motorcycleId,
    p_item_ids: record.itemIds,
    p_performed_date: record.performedDate,
    p_odometer_km: record.odometerKm,
    p_cost_sen: record.costSen ?? null,
    p_provider: record.provider || null,
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      current_odometer_km: Math.max(
        0,
        Math.round(Number(bike.currentOdometerKm) || 0),
      ),
    })
    .eq("id", motorcycleId)
    .eq("user_id", userId)
    .select("id,name,make,model,start_date,current_odometer_km")
    .single();
  if (error) throw error;
  return {
    id: data.id,
    ...
    currentOdometerKm: Number(data.current_odometer_km),
  } as Motorcycle & { id: string };
}
